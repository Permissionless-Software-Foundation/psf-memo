...
Current `master` HEAD: `3045520` (the docs-only comment commit re-applied on
`bd86490`, the `recent-profile-follow-confirm` architect records commit). The
client record names the architect code review commit `20b13f2e3f`; the only
later commits are `bd86490` (record + summary) and `3045520` (the feature
comment), both docs-only, so the record is valid for the merged tree. This task
adds a confirmation step before the follow/unfollow broadcast: clicking a
`/profile/recent` Follow/Unfollow button opens a modal asking "Are you sure you
want to follow <display name>?" (or "unfollow") with Yes and No. No closes the
modal without broadcasting and leaves the row unchanged; Yes continues to the
existing loading -> success (message + txid + `bch.loping.net` explorer link)
or red-error flow. The display name is the profile's name, or the truncated
address, matching the Account column (#52). Client-only: no DB/indexer change.
The same feature file (`psf-memo-client/specs/recent-profile-follow.feature`)
was extended in place (scenarios 5-11); the tool-written mutation manifest was
regenerated by the architect. Record:
`docs/reviews/recent-profile-follow-confirm-verification.json` (client);
architect summary `docs/reviews/recent-profile-follow-confirm-summary.md`. The
specifier merged and ran only the merged feature's acceptance test (13/13
example executions) as the independent check. `verify.sh client` was pass 5/5
at `20b13f2` (unit 563/0, property 146/0, acceptance 39 suites, lint ok, build
ok); language mutation 18 killed / 0 survived on `recent-profiles-page.js`;
soft Gherkin mutation 13/11 killed with the 2 intrinsic survivors in #52.

Merge note: a docs-only spec-comment commit (`15a022b`) had landed on `master`
after the spec commit `6f3f515`, so the architect branch (based on `6f3f515`)
was not fast-forwardable. Rather than add a merge commit, the specifier reset
to the merge base, fast-forwarded to `bd86490`, and cherry-picked the comment
as `3045520`, keeping history linear. The record-vs-tip diff is docs/tool
metadata only.

Previous `master` HEAD before this merge: `6f3f515` (the confirmation spec)
with `15a022b` on top. The preceding `recent-profile-follow` merge (TXID column
-> Follow, button, result modal) is recorded in
`specs/feature-backlog.md`. `master` also still carries four human commits made
outside the swarm pipeline - `f7809d0` (feed-page button styling), `477c1c1`
(recent-profiles page info), `ea67979` (removed the redundant inline author name
from feed posts), and `8eab46d` (Notifications entry CSS/layout). None is
covered by a Gherkin spec; reconcile if a future feature touches those
surfaces.
