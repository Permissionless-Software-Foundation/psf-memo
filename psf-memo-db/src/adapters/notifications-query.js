...
class NotificationsQuery {
  ...
  async _replyNotificationChild (child, addr) {
    const parent = await this._getPostOrNull(child.parentTxid)
    if (!parent || parent.addr !== addr) return null

    const childPost = await this._getPostOrNull(child.childTxid)
